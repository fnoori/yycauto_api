...
exports.addNewVehicle = (req, res, next) => {
  ...
  if (req.user[process.env.DEALERSHIP_ID_METADATA].dealershipId != req.params.dealershipId) {
    if (req.files) {
  ...
  Dealership.findById(req.user[process.env.DEALERSHIP_ID_METADATA].dealershipId)
    .populate('Dealership')
    .select('AccountCredentials.Email Name Phone Address _id')
    .exec().then(dealershipResult => {
      if (dealershipResult) {
        var vehicleData = {};
        ...
        .then(saveResult => {
          Dealership.findById(req.user[process.env.DEALERSHIP_ID_METADATA].dealershipId)
          .exec().then(dealershipSaved => {
            const dealershipFolder = dealershipSaved['name'].split(' ').join('_');
            ...
            for (var i = 0; i < req.files.length; i++) {
              ...
              googleBucketReqs.storage
                ...
                .then((googleBucketUploadSuccess) => {
                  utilities.emptyDir(rootTempVehicleDir);
                }).catch(bucketUploadErr => {
                  console.log('bucketUploadErr', bucketUploadErr);
            ...
            console.log('dealershipFindByIdErr', dealershipFindByIdErr);
            utilities.emptyDir(rootTempVehicleDir);
          ...
          console.log('newVehicleSaveErr', newVehicleSaveErr);
          utilities.emptyDir(rootTempVehicleDir);
...
exports.deleteVehicle = (req, res, next) => {
  ...
  if (req.user[process.env.DEALERSHIP_ID_METADATA].dealershipId != dealershipId) {
